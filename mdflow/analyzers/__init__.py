...
class StructureAnalyzer:
    ...
    def heading_tree(self, doc: MdDocument) -> list[dict]:
        ...
        for h in doc.headings:
            node = {"level": h.level, "text": h.text, "anchor": h.anchor, "children": []}
    ...
    def section_summary(self, doc: MdDocument) -> list[dict]:
        ...
        for i, h in enumerate(headings):
            next_line = headings[i + 1].line if i + 1 < len(headings) else float("inf")
            cbs = [cb for cb in doc.code_blocks
                   if h.line <= cb.line_start < next_line]
            lks = [l for l in doc.links if h.line <= l.line < next_line]
